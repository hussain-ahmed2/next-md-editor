...
type BlockSerializer = (block: Block) => string;

interface SerializeCacheEntry {
  indentLevel: number;
  /**
   * The custom serializer that produced `value`, or undefined if the
   * built-in switch did. Stored so a cache entry created *before*
   * `initRegistry()` ran (when custom serializers are not yet registered)
   * is detected as stale and recomputed, instead of returning markdown
   * generated by the wrong code path.
   */
  serializer: BlockSerializer | undefined;
  value: string;
}

/**
 * Blocks are immutable — the editor store replaces the object for the one
 * block that changed — so identity is a safe cache key. This turns a
 * whole-document re-serialize (which runs on every preview update) into
 * work proportional to the number of edited blocks.
 */
const serializeCache = new WeakMap<Block, SerializeCacheEntry>();
...
function serializeBlock(
  ...
  const resolvedSerializer = lookupSerializer?.(block.type);
  const cached = serializeCache.get(block);
  if (
    cached &&
    cached.indentLevel === indentLevel &&
    cached.serializer === resolvedSerializer
  ) {
    return cached.value;
  }
  const value = serializeBlockUncached(block, indentLevel, lookupSerializer);
  serializeCache.set(block, { indentLevel, serializer: resolvedSerializer, value });
  return value;
}

function serializeBlockUncached(
  block: Block,
  indentLevel: number = 0,
  lookupSerializer?: SerializerLookup,
): string {
