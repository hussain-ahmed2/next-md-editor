...
function splitRichTextAt(rt: RichText, offset: number): { before: RichText; after: RichText } {
...
function applyRichFormat(rt: RichText, start: number, end: number, format: FormatFlags): RichText {
...
function toggleRichFormat(rt: RichText, start: number, end: number, format: FormatFlags): RichText {
	if (start >= end) return rt;
...
function getFormatKey(f: FormatFlags): string {
	...
	return (["bold", "italic", "strikethrough", "link"] as const).filter((k) => f[k]).join(",");
}
...
function deleteRichRange(rt: RichText, start: number, end: number): RichText {
	if (start >= end) return rt;
	const { before: left, after: midAfter } = splitRichTextAt(rt, start);
...
function restoreDomRange(container: HTMLElement, startOffset: number, endOffset: number) {
	const walker = document.createTreeWalker(container, NodeFilter.SHOW_TEXT);
	let pos = 0;
...
function getSelectionRichRange(rt: RichText, container: HTMLElement): { start: number; end: number } | null {
	const sel = window.getSelection();
...
function richTextToHtml(rt: RichText): string {
	...
	// Append zero-width space at the end to ensure contentEditable always has an
	// unformatted text node at the boundary. This allows users to type space after
	// a bold/italic/code element and break out of it naturally.
	return html + "&#8203;";
}
	...
	function process(parent: HTMLElement, inherited: FormatFlags[]): RichText {
		...
		for (const child of parent.childNodes) {
			if (child.nodeType === Node.TEXT_NODE) {
				...
				// Strip out ZWSP so it doesn't pollute the RichText state
				text = text.replace(/\u200B/g, "");
				if (!text) continue;
